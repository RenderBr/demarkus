...
# V2 does not accept V1 config.command mutations. It discovers Markdown
# commands directly; leave a user's command with the same name untouched.
for command in "${ASSETS_DEST}/commands/"*.md; do
  name="$(basename "${command}")"
  dest="${COMMAND_DEST}/${name}"
  if [[ -e "${dest}" ]] && ! grep -q 'Code generated by tools/plugin-prompts' "${dest}"; then
